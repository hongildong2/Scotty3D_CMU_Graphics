...
struct Framebuffer {
  ...
  //  - having a limited size makes it easier to write rasterization functions
  //  with fixed-point
  //    math (if you want)
  //  - having an even size avoids some corner cases if you choose to rasterize
  //  with quadfrags
  ...
  uint32_t index(uint32_t x, uint32_t y, uint32_t s) const {
    ...
    return y * width + x + s * (width * height);
  }
