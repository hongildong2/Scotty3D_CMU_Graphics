...
Framebuffer::Framebuffer(uint32_t width_, uint32_t height_,
  ...
  if (width % 2 != 0 || height % 2 != 0) {
    throw std::runtime_error("Framebuffer size (" + std::to_string(width) +
                             "x" + std::to_string(height) + ") is not even.");
  }

  uint32_t samples =
      width * height *
      static_cast<uint32_t>(sample_pattern.centers_and_weights.size());
...
HDR_Image Framebuffer::resolve_colors() const {
  ...
  for (uint32_t y = 0; y < height; ++y) {
    for (uint32_t x = 0; x < width; ++x) {
      ...
      for (int s = 0; s < sample_pattern.centers_and_weights.size(); ++s) {
        const auto &pattern = sample_pattern.centers_and_weights[s];
        const Vec2 &offset = pattern.xy();
        const float weight = pattern.z;
        image.at(x, y) += weight * color_at(x + offset.x, y + offset.y, s);
      }
    }
  }
