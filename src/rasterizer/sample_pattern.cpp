...
std::vector<SamplePattern> const &SamplePattern::all_patterns() {
  ...
  auto Grid = [](uint32_t w, uint32_t h = -1U, uint32_t id = -1U) {
    ...
    for (uint32_t y = 0; y < h; ++y) {
      for (uint32_t x = 0; x < w; ++x) {
        centers_and_weights.emplace_back((x + 0.5f) / w, (y + 0.5f) / h,
                                         weight);
      }
    }
    return SamplePattern(
        id, "Grid (" + std::to_string(w) + "x" + std::to_string(h) + ")",
        centers_and_weights);
  };
  ...
  auto CreateYourOwnSamplePattern = []() {
    ...
    // Make any (literally any) SamplePattern that isn't one of the existing
    // ones provided:
    // ("Center", "Grid 2x2", "Grid 4x4", "Grid 8x8").
    //
    // In your writeup, explain what your SamplePattern does and provide
    // reasoning on what scenarios your SamplePattern would do well in vs do
    // horribly in. The more complicated your SamplePattern is + more detailed
    // your writeup is (such as with images or models for comparisons of these
    // scenarios), the more extra credit can be awarded (no limit).
    ...
    std::vector<Vec3> centers_and_weights = {
        Vec3(-0.01f, -0.01f, 0.25f), Vec3(-0.01f, 0.01f, 0.25f),
        Vec3(0.01f, -0.01f, 0.25f), Vec3(0.01f, 0.01f, 0.25f)};
