...
// Scene's introspection system is built on each relevant item in a Scene
// providing either:
...
auto introspect_impl(F &&f, T &&t, int)
    -> decltype(std::remove_reference<T>::type::template introspect<I>(f, t)) {
  std::remove_reference<T>::type::template introspect<I>(std::forward<F>(f),
                                                         std::forward<T>(t));
}
...
template <Intent I, typename F, typename T> void introspect(F &&f, T &&t) {
#ifdef INTROSPECT_SOFT_FAILURE
...
// on write - introspects the type, updates the alternative, then introspects
// the new alternative

template <Intent I, typename F, typename V,
          /*size_t sz = std::variant_size_v< std::remove_reference_t< V > >,*/
          std::enable_if_t<I == Intent::Read || I == Intent::Animate, bool> =
              true>
...
// base case:
template <Intent I, size_t idx, typename V, typename F,
          std::enable_if_t<idx >= std::variant_size_v<clean_t<V>>, bool> = true>
void write_type(F &&f, V &&v, std::string const &type) {
  ...
  warn("Type '%s' does not appear in variant -- will substitute '%s'.",
       type.c_str(), T::TYPE);
...
// inductive case:
template <Intent I, size_t idx, typename V, typename F,
          std::enable_if_t<idx<std::variant_size_v<clean_t<V>>, bool> =
                               true> void write_type(F &&f, V &&v,
...
template <Intent I, typename F, typename V,
          std::enable_if_t<I == Intent::Write, bool> = true>
void introspect_variant(F &&f, V &&v) {
  std::string type;
  std::visit(
      [&](auto &&t) { type = std::remove_reference_t<decltype(t)>::TYPE; }, v);
  f("type", type);
...
// translates enum to string using table, introspects the string, converts back
// to enum using table

template <Intent I, typename F, typename E, typename E2>
void introspect_enum(F &&f, const char *name, E &&e,
                     std::vector<std::pair<const char *, E2>> const &possible) {
  assert(!possible.empty());

  std::string value;
  { // find current name from supplied list:
    auto found = std::find_if(possible.begin(), possible.end(),
                              [&](auto &&p) { return p.second == e; });
  ...
  if constexpr (I == Intent::Write) {
    ...
    if (found == possible.end()) {
      warn("Invalid enum value '%s' (for %s); setting to '%s'.", value.c_str(),
           name, possible[0].first);
