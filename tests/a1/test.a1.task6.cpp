...
// build a testing mipmap where (r,g) have sample texcoord and (b) has level
// index:
[[maybe_unused]] static Textures::Image test_mipmap_texture() {
  Textures::Image ret;
  ...
  auto set_colors = [](HDR_Image &image, uint32_t level) {
    for (uint32_t y = 0; y < image.h; ++y) {
      for (uint32_t x = 0; x < image.w; ++x) {
        image.at(x, y) =
            Spectrum((x + 0.5f) / image.w, (y + 0.5f) / image.h, float(level));
      }
    }
  };
...
Spectrum sample_trilinear(HDR_Image const &image,
...
Test test_a1_task6_nearest_simple("a1.task6.sample.nearest.simple", []() {
  ...
  HDR_Image image(
      3, 5, std::vector<Spectrum>{R, R, R, G, G, G, R, B, R, G, W, G, B, B, B});

  auto expect_spectrum = [&](std::string const &desc, Vec2 uv,
                             Spectrum expected) {
...
Test test_a1_task6_bilinear_simple("a1.task6.sample.bilinear.simple", []() {
  ...
  // expect_spectrum("at center", px_to_uv * Vec2(1.5f, 3.5f), W);
  // expect_spectrum("linear x", px_to_uv * Vec2(1.25f, 2.5f), Spectrum(0.25f,
  // 0.0f, 0.75f)); expect_spectrum("linear y", px_to_uv * Vec2(0.5f, 2.75f),
  // Spectrum(0.75f, 0.25f, 0.0f));
  expect_spectrum("bilinear xy", px_to_uv * Vec2(2.4f, 3.25f),
                  Spectrum(0.3f, 0.75f, 0.1f));
});
...
Test test_a1_task6_trilinear_simple("a1.task6.sample.trilinear.simple", []() {
  ...
  HDR_Image image(
      3, 5, std::vector<Spectrum>{R, R, R, R, R, R, R, R, R, R, R, R, R, R, R});
  std::vector<HDR_Image> levels;
  levels.emplace_back(1, 2, std::vector<Spectrum>{G, G});
  levels.emplace_back(1, 1, std::vector<Spectrum>{B});

  auto expect_spectrum = [&](std::string const &desc, Vec2 uv, float lod,
                             Spectrum expected) {
  ...
  expect_spectrum("base - levels[0]", Vec2(0.5f, 0.5f), 0.25f,
                  Spectrum(0.75f, 0.25f, 0.0f));
  expect_spectrum("levels[0] - levels[1]", Vec2(0.5f, 0.5f), 1.7f,
                  Spectrum(0.0f, 0.3f, 0.7f));
});
...
Test test_a1_task6_generate_mipmap("a1.task6.generate_mipmap", []() {
  HDR_Image image(4, 6,
                  std::vector<Spectrum>{R, R, B, B, R, R, B, B, B, B, R, R,
                                        B, B, R, R, R, R, B, B, R, R, B, B});
  ...
  if (levels.size() != expected.size()) {
    throw Test::error("Image of size " + std::to_string(image.w) + "x" +
                      std::to_string(image.h) + " should have " +
                      std::to_string(expected.size()) +
                      " levels, but generated " +
  ...
  for (uint32_t l = 0; l < expected.size(); ++l) {
    if (levels[l].w != expected[l].first || levels[l].h != expected[l].second) {
      throw Test::error(
          "Image of size " + std::to_string(image.w) + "x" +
          std::to_string(image.h) + " should have levels[" + std::to_string(l) +
          "] of size " + std::to_string(expected[l].first) + "x" +
          std::to_string(expected[l].second) + " but generated level of size " +
          std::to_string(levels[l].w) + "x" + std::to_string(levels[l].h) +
          ".");
    }
  }
  ...
  if (Test::differs(levels.back().at(0, 0), Spectrum(0.5f, 0.0f, 0.5f))) {
    ...
    info("%s", params.c_str());
    throw Test::error(
        "Mipmap generation didn't approximately average image in last level.");
  }
});
...
Test test_a1_task6_lod_simple("a1.task6.lod.simple", []() {
  ...
  std::array<float, 5> attribs;
  attribs.fill(0.0f);
  // texture coordinate (0.5f, 0.5f):
  attribs[L::FA_TexCoordU] = 0.5f;
  attribs[L::FA_TexCoordV] = 0.5f;
  // normal pointing straight up:
  attribs[L::FA_NormalX] = 0.0f;
  attribs[L::FA_NormalY] = 0.0f;
  attribs[L::FA_NormalZ] = 1.0f;

  std::array<Vec2, 2> derivs;
  derivs.fill(Vec2{0.0f, 0.0f});

  auto expect_lod = [&](std::string const &desc, Vec2 du, Vec2 dv,
    ...
    if (!(min_lod <= lod && lod <= max_lod)) {
      ...
      params += "  expected: " + std::to_string(min_lod) +
                " <= lod <= " + std::to_string(max_lod) + "\n";
  ...
  Vec2 px_to_texcoord =
      Vec2(1.0f / test_texture.image.w, 1.0f / test_texture.image.h);

  expect_lod("zero texels per pixel", px_to_texcoord * Vec2(0.0f, 0.0f),
             px_to_texcoord * Vec2(0.0f, 0.0f), -0.001f, 0.001f);
  // larger leeway here given how many methods one might come up with.
  //     but should generally be closer to level zero than not!
  expect_lod("one texels per pixel", px_to_texcoord * Vec2(1.0f, 0.0f),
             px_to_texcoord * Vec2(0.0f, 1.0f), -0.001f, 0.501f);
  // covering two texels, should definitely be into level 1:
  expect_lod("two texels per pixel", px_to_texcoord * Vec2(2.0f, 0.0f),
             px_to_texcoord * Vec2(0.0f, 2.0f), 0.5f, 1.5f);
  // covering four texels, should be into level 2:
  expect_lod("four texels per pixel", px_to_texcoord * Vec2(4.0f, 0.0f),
             px_to_texcoord * Vec2(0.0f, 4.0f), 1.5f, 2.5f);
});
