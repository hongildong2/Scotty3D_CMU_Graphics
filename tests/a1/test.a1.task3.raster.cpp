...
using FlatPipeline = Pipeline<PrimitiveType::Triangles, Programs::Lambertian,
                              Pipeline_Blend_Replace | Pipeline_Depth_Less |
                                  Pipeline_Interp_Flat>;
using FPClippedVertex = FlatPipeline::ClippedVertex;
using FPFragment = FlatPipeline::Fragment;

[[maybe_unused]] static std::string to_string(FPClippedVertex const &vert) {
  std::string ret = "{"
                    " .fb_position = (" +
                    std::to_string(vert.fb_position.x) + ", " +
                    std::to_string(vert.fb_position.y) + ", " +
                    std::to_string(vert.fb_position.z) +
                    ")"
                    ", .inv_w = " +
                    std::to_string(vert.inv_w);
  ret += ", .attributes = [";
  for (auto const &a : vert.attributes) {
    if (&a != &vert.attributes[0])
      ret += ", ";
    ret += std::to_string(a);
  }
  ret += "] }";
...
[[maybe_unused]] static std::string to_string(FPFragment const &frag) {
  std::string ret = "{"
                    " .fb_position = (" +
                    std::to_string(frag.fb_position.x) + ", " +
                    std::to_string(frag.fb_position.y) + ", " +
                    std::to_string(frag.fb_position.z) + ")";
  ret += ", .attributes = [";
  for (auto const &a : frag.attributes) {
    if (&a != &frag.attributes[0])
      ret += ", ";
    ret += std::to_string(a);
  }
  ret += "]";
...
static void check_rasterize_triangles(
    std::string const &desc,
    std::initializer_list<FlatPipeline::ClippedVertex> const &vertices_,
    std::initializer_list<FlatPipeline::Fragment> const &expected_) {
  std::vector<FlatPipeline::ClippedVertex> vertices(vertices_);
  std::vector<FlatPipeline::Fragment> expected(expected_);

  if (vertices.size() % 3 != 0) {
    throw Test::error(
        "Example '" + desc + "' is INVALID because the vertex count (" +
        std::to_string(vertices.size()) + ") is not divisible by three.");
  }

  for (auto const &v : vertices) {
    if (v.fb_position.x < 0.0f ||
        v.fb_position.x > float(Framebuffer::MaxWidth) ||
        v.fb_position.y < 0.0f ||
        v.fb_position.y > float(Framebuffer::MaxHeight) ||
        v.fb_position.z < 0.0f || v.fb_position.z > 1.0f) {
      throw Test::error(
          "Example '" + desc +
          "' is INVALID because it includes a vertex with fb_position " +
          to_string(v.fb_position) +
          ", which lies outside the largest possible framebuffer [0," +
          std::to_string(Framebuffer::MaxWidth) + "]x[0," +
          std::to_string(Framebuffer::MaxHeight) + "]x[0,1].");
    }
  }
  ...
  for (uint32_t i = 2; i < vertices.size(); i += 3) {
    ...
    FlatPipeline::rasterize_triangle(vertices[i - 2], vertices[i - 1],
                                     vertices[i], emit_fragment);
  }
  ...
  for (uint32_t gi = 0; gi < got.size(); ++gi) {
    ...
    for (uint32_t i = 0; i < expected.size(); ++i) {
      ...
      // check for exact position match, since positions are always exactly
      // representable as floating point:
      if (!(g.fb_position.x == e.fb_position.x &&
            g.fb_position.y == e.fb_position.y))
        continue;
      position_matches.emplace_back(i);
      // check z and attribs and derivatives (approximately) match:
      if (Test::differs(g.fb_position.z, e.fb_position.z))
        continue;
      std::function<bool(float, float)> attribute_close = [](float a, float b) {
        return !Test::differs(a, b);
      };
      std::function<bool(Vec2, Vec2)> derivative_close = [](Vec2 a, Vec2 b) {
        return !Test::differs(a, b);
      };
      // check attributes (approximately) match:
      if (!std::equal(g.attributes.begin(), g.attributes.end(),
                      e.attributes.begin(), e.attributes.end(),
                      attribute_close))
        continue;
      // check derivatives (approximately) match:
      if (!std::equal(g.derivatives.begin(), g.derivatives.end(),
                      e.derivatives.begin(), e.derivatives.end(),
                      derivative_close))
        continue;
    ...
    if (found_index != -1U) {
      ...
      if (!position_matches.empty()) {
        // not found, but some other fragments with different z or attribs were
        // here.
        ++wrong_data;
        notes[gi] = " [WRONG DATA]";
      } else {
  ...
  auto dump_info = [&]() {
    ...
    for (uint32_t i = 2; i < vertices.size(); i += 3) {
      ...
      std::cout << "  rasterize_triangle(v[" << i - 2 << "],v[" << i - 1
                << "],v[" << i << "]) emitted " << count << " fragments:\n";
      for (auto const &f : got) {
        uint32_t fi = uint32_t(&f - &got[0]);
        if (f.id == id) {
          std::cout << "    f[" << (&f - &got[0])
                    << "]: " << to_string(f.fragment) << notes[fi] << '\n';
        }
      }
    }
    std::cout << "  (for a total of " << got.size() << " fragments from "
    ...
    std::cout << "  " << matches << "/" << expected.size()
              << " fragments are correct;";
    std::cout << " " << missing << " fragments are missing;";
    std::cout << " " << overlaps << " fragments are overlapped/duplicated;";
    std::cout << " " << wrong_position
              << " fragments are in unexpected positions;";
    std::cout
        << " and " << wrong_data
        << " fragments are in expected positions but have the wrong data.\n";
  ...
  if (wrong_data) {
    dump_info();
    throw Test::error("Example '" + desc +
                      "' had fragments with incorrect data");
  }
...
Test test_a1_task3_raster_thin_2px("a1.task3.raster.thin.2px", []() {
  check_rasterize_triangles(
      "thin triangle exceeding [2,3]x[1,3] but only covering (2.5,1.5) and "
      "(2.5,2.5)",
